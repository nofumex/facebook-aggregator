...
On first use the configured order is intersected with IDs returned by authenticated `GET /models`; unavailable IDs are never called. Network, timeout, 429 and 5xx errors retry the same model with bounded 500ms/1s/2s exponential backoff. Malformed JSON, invalid types/enums/ranges and contradictions immediately advance to the next model. Compatible `json_object` responses may omit nullable fields: the application fills the canonical schema with null/Unknown before validation and caching. Structured logs include post ID, model, attempt, latency, validation/fallback reason and token usage. A valid first result stops the cascade.
